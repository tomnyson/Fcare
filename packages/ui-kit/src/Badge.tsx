...
type BadgeTone = 'info' | 'success' | 'warning' | 'orange' | 'danger' | 'neutral';
...
const TONE_CLASSES: Record<BadgeTone, string> = {
  ...
  // Mức cảnh báo 3 — nằm giữa vàng (mức 2) và đỏ (mức 4).
  orange: 'bg-fpt-orange-50 text-fpt-orange-600',
...
const PULSE_CLASSES: Record<BadgeTone, string> = {
  ...
  orange: 'animate-pulse ring-2 ring-fpt-orange/40 shadow-sm',
    ...
    >
      {pulse && (
        <span className="relative mr-1.5 flex h-2 w-2">
          ...
          />
          <span className={`relative inline-flex h-2 w-2 rounded-full ${DOT_PULSE_COLORS[tone]}`} />
        </span>
      )}
      {children}
    </span>
