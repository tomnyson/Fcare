...
- **Cảnh báo điểm danh tự động** (`source = AUTO_ATTENDANCE`, `raisedById = null`): sau mỗi lần import/commit điểm danh, `AttendanceReviewService` rà soát `absentSessions` lũy kế của enrollment — `== 2` → cấp 2, `>= 3` → cấp 3, không bao giờ tự lên cấp 4. Cùng (SV, lớp học phần, kỳ) chỉ có 1 cảnh báo mở; số vắng tăng làm **thay thế** (resolve cảnh báo cũ + tạo cảnh báo mới, `ownerCaredAt` reset, gửi lại thông báo theo ma trận), KHÔNG sửa `level` tại chỗ. `ownerCaredAt` chỉ được set khi **chính giảng viên đứng lớp** (`classSection.lecturerId`) ghi `CareLog` gắn `alertId`; GV khác ghi nhật ký vẫn được nhưng banner của GV đứng lớp vẫn hiện. Banner "đeo" ở Dashboard lấy từ `GET /attendance-alerts/pending?term&scope=owned|all` (đi qua `studentScope`); thống kê chăm sóc có 4 cột `attendanceAlerts.{total,caredByOwner,caredByOthers,pending}`. Rà soát tay: `POST /attendance-alerts/review` (TRAINING_OFFICER/ADMIN).
...
- **Phân trang**: API trả `{ items, meta: { total, page, limit } }` (`/students`, `/alerts`, `/imports`, `/imports/:id/preview?page&limit&onlyErrors`) — KHÔNG trả nguyên mảng cho bảng có thể vượt vài trăm dòng. Web: trang có URL-as-state dùng `parsePageParam` + `<Pagination>` (`components/ui/pagination.tsx`), đổi bộ lọc phải reset `page`; danh mục đã tải hết (môn học, lớp học phần, thẻ lớp) cắt trang phía web bằng `usePagedList`/`useCatalogPaging`.
...
This project is indexed by GitNexus as **Fcare** (3859 symbols, 10144 relationships, 300 execution flows). Use the GitNexus MCP tools to understand code, assess impact, and navigate safely.
